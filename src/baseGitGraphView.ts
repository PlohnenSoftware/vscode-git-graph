    ...
    this.logger.log(
      "Created Git Graph View" +
        (this.loadViewTo !== null
          ? " (active repo: " + this.loadViewTo.repo + ")"
          : "")
    );
  }
  ...
  private async respondToMessage(msg: RequestMessage) {
    ...
    switch (msg.command) {
      ...
      case "dropCommits":
        this.sendMessage({
          command: "dropCommits",
          error: await this.dataSource.dropCommits(
            msg.repo,
            (msg as RequestDropCommits).commits
          ),
        });
        break;
      case "dropStash":
      ...
      case "loadCommits":
        ...
        const refs =
          msg.branches === null && msg.tags === null
            ? null
            : (msg.branches || []).concat(msg.tags || []);
        this.sendMessage({
          command: "loadCommits",
