...
export class DataSource extends Disposable {
  ...
  public async editCommitMessage(
    ...
    try {
      const headCommit = await this.spawnGit(
        ["rev-parse", "HEAD"],
        repo,
        (stdout) => stdout.trim()
      );
  ...
  private async rebaseEditCommitMessage(
    ...
    const parentCommit = await this.spawnGit(
    ...
    return new Promise<ErrorInfo>((resolve) => {
      ...
      const env = Object.assign({}, process.env, this.askpassEnv, {
        GIT_SEQUENCE_EDITOR: `sed -i.bak "s/^pick ${commitHash.substring(
          0,
          7
        )}/reword ${commitHash.substring(0, 7)}/" "$1"`,
        GIT_EDITOR: `sh -c 'echo '"'"'${escapedMessage}'"'"' > "$@"' -- `,
      });

      resolveSpawnOutput(
        cp.spawn(this.gitExecutable.path, args, { cwd: repo, env })
      )
        .then(([status, stdout, stderr]) => {
          resolve(
            status.code !== 0
              ? getErrorMessage(status.error, stdout, stderr)
              : null
          );
        })
        .catch((errorMessage) => {
